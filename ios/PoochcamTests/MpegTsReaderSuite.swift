import AVFoundation
@testable import Poochcam
import Testing

private class Mock: MpegTsReaderDelegate {
    var audioSampleBuffers: [CMSampleBuffer] = []
    var videoSampleBuffers: [CMSampleBuffer] = []
    var targetLatencies: [(Double, Double)] = []

    func mpegTsReaderAudioBuffer(_ sampleBuffer: CMSampleBuffer) {
        audioSampleBuffers.append(sampleBuffer)
    }

    func mpegTsReaderVideoBuffer(_ sampleBuffer: CMSampleBuffer) {
        videoSampleBuffers.append(sampleBuffer)
    }

    func mpegTsReaderSetTargetLatencies(_ audioLatency: Double, _ videoLatency: Double) {
        targetLatencies.append((audioLatency, videoLatency))
    }
}

// swiftlint:disable line_length

struct MpegTsReaderSuite {
    // aevalsrc=exprs='if(lt(mod(t,1),0.015),0.8*sin(2*PI*1800*t)*exp(-80*mod(t,1)),0)':s=48000
    @Test
    func ffmpegAudioOnlyPeriodicBeep() throws {
        let mock = Mock()
        let reader = MpegTsReader(name: "",
                                  decoderQueue: .main,
                                  timecodesEnabled: false,
                                  softwareDecoding: false,
                                  targetLatency: 1.0)
        reader.delegate = mock
        var packet =
            try Data(
                hexString: ""
            )
        try reader.handlePacketFromClient(packet: packet)
        #expect(mock.audioSampleBuffers.count == 0)
        packet =
            try Data(
                hexString: ""
            )
        try reader.handlePacketFromClient(packet: packet)
        #expect(mock.audioSampleBuffers.count == 0)
        packet =
            try Data(
                hexString: ""
            )
        try reader.handlePacketFromClient(packet: packet)
        #expect(mock.audioSampleBuffers.count == 17)
        packet =
            try Data(
                hexString: ""
            )
        try reader.handlePacketFromClient(packet: packet)
        #expect(mock.audioSampleBuffers.count == 34)
        packet =
            try Data(
                hexString: ""
            )
        try reader.handlePacketFromClient(packet: packet)
        #expect(mock.audioSampleBuffers.count == 34)
        packet =
            try Data(
                hexString: ""
            )
        try reader.handlePacketFromClient(packet: packet)
        #expect(mock.audioSampleBuffers.count == 51)
        packet =
            try Data(
                hexString: ""
            )
        try reader.handlePacketFromClient(packet: packet)
        #expect(mock.audioSampleBuffers.count == 68)
        packet =
            try Data(
                hexString: ""
            )
        try reader.handlePacketFromClient(packet: packet)
        #expect(mock.audioSampleBuffers.count == 85)
        packet =
            try Data(
                hexString: ""
            )
        try reader.handlePacketFromClient(packet: packet)
        #expect(mock.audioSampleBuffers.count == 85)
        packet =
            try Data(
                hexString: ""
            )
        try reader.handlePacketFromClient(packet: packet)
        #expect(mock.audioSampleBuffers.count == 102)
        packet =
            try Data(
                hexString: ""
            )
        try reader.handlePacketFromClient(packet: packet)
        #expect(mock.audioSampleBuffers.count == 119)
        #expect(mock.videoSampleBuffers.count == 0)
        #expect(mock.targetLatencies.isEmpty)
        for audioSampleBuffer in mock.audioSampleBuffers {
            let audioStreamBasicDescription = try #require(audioSampleBuffer.formatDescription?
                .audioStreamBasicDescription)
            #expect(audioStreamBasicDescription.mSampleRate == 48000)
            #expect(audioStreamBasicDescription.mFormatID == 0x6C70_636D) // lpcm
            #expect(audioStreamBasicDescription.mFormatFlags == 0xC)
            #expect(audioStreamBasicDescription.mBytesPerPacket == 2)
            #expect(audioStreamBasicDescription.mFramesPerPacket == 1)
            #expect(audioStreamBasicDescription.mBytesPerFrame == 2)
            #expect(audioStreamBasicDescription.mChannelsPerFrame == 1)
            #expect(audioStreamBasicDescription.mBitsPerChannel == 16)
        }
        for audioSampleBuffer in mock.audioSampleBuffers {
            #expect(audioSampleBuffer.totalSampleSize == 1024 * 2)
        }
        #expect(mock.audioSampleBuffers[0].dataBuffer?.data != Data(count: 2048))
        #expect(mock.audioSampleBuffers[1].dataBuffer?.data != Data(count: 2048))
        for index in 2 ... 46 {
            #expect(mock.audioSampleBuffers[index].dataBuffer?.data == Data(count: 2048),
                    "At index: \(index)")
        }
        #expect(mock.audioSampleBuffers[47].dataBuffer?.data != Data(count: 2048))
        #expect(mock.audioSampleBuffers[48].dataBuffer?.data != Data(count: 2048))
        for index in 49 ... 93 {
            #expect(mock.audioSampleBuffers[index].dataBuffer?.data == Data(count: 2048),
                    "At index: \(index)")
        }
        #expect(mock.audioSampleBuffers[94].dataBuffer?.data != Data(count: 2048))
        #expect(mock.audioSampleBuffers[95].dataBuffer?.data != Data(count: 2048))
        for index in 96 ... 118 {
            #expect(mock.audioSampleBuffers[index].dataBuffer?.data == Data(count: 2048),
                    "At index: \(index)")
        }
        let firstPresentationTimeStamp = try #require(mock.audioSampleBuffers.first?.presentationTimeStamp
            .seconds)
        #expect(areEqual(mock.audioSampleBuffers.map(\.presentationTimeStamp.seconds),
                         (0 ... 118).map { firstPresentationTimeStamp + Double($0) * 0.02133333 },
                         epsilon: 0.001))
    }
}

// swiftlint:enable line_length
