...
struct WidgetBrowserSettingsView: View {
    ...
    var body: some View {
        ...
        Section {
            Toggle("Poochcam access", isOn: $browser.poochcamAccess)
                .onChange(of: browser.poochcamAccess) { _ in
                    model.resetSelectedScene(changeScene: false)
                }
            if browser.poochcamAccess {
            ...
            Text(
                "Give the webpage access to various data in Poochcam, for example chat messages and your location."
            )
        }
