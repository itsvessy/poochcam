...
extension Model {
    func startStream(delayed: Bool = false) {
        ...
        if PoochcamProfile.enabled {
            guard let configuration = poochcamConfiguration.configuration,
                  (try? configuration.validated()) != nil else { return }
            poochcamPublishingURL = configuration.publishingURL
        }
        if delayed, !isLive {
            return
        }
    ...
    private func startNetStreamRtmp() {
        ...
        let url: String
        if PoochcamProfile.enabled {
            guard let publishingURL = poochcamPublishingURL else { return }
            url = publishingURL
        } else { url = stream.url }
        media.rtmpStartStream(url: url,
    ...
    private func startNetStreamSrt() {
        ...
        media.srtStartStream(
            ...
            adaptiveBitrateAlgorithm: srt.adaptiveBitrateEnabled
                ...
                : nil,
            latency: srt.latency,
            experimental: database.debug.enhancedPoochcamSrt,
    ...
    private func handleRtmpDisconnected(message: String) {
        onDisconnected(reason: PoochcamProfile.enabled ? "RTMP connection closed" : "RTMP disconnected with message \(message)")
    }
    ...
    private func handleAttachCameraError() {
        DispatchQueue.main.async {
            self.poochcamCaptureError = "Camera unavailable. Close other camera apps, then reopen Poochcam."
        }
        makeErrorToastMain(
    ...
    private func handleCaptureSessionError(message: String) {
        DispatchQueue.main.async {
            self.poochcamCaptureError = "Camera interrupted. Keep Poochcam open; reopen it if the view does not return."
        }
