...
extension Model {
    ...
    func setupAudio() {
        ...
        if PoochcamProfile.enabled {
            let builtIn = database.mics.mics.filter { $0.connected && $0.isBuiltin() }
            defaultMic = builtIn.first(where: { $0.builtInOrientation == .back }) ?? builtIn.first ?? noMic
        ...
        processorControlQueue.async {
            ...
            do {
                ...
                try session.setCategory(
                    .playAndRecord,
                    options: PoochcamProfile.enabled ? [.defaultToSpeaker] : [.mixWithOthers, bluetoothOption, .defaultToSpeaker]
                )
                ...
                DispatchQueue.main.async {
                    self.poochcamCaptureError = "Microphone unavailable. Close other audio apps, then reopen Poochcam."
