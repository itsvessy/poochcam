...
private func randomBuyIconsTitle() -> String {
    [
        String(localized: "👍 Buy Poochcam icons if you like the app 👍"),
        String(localized: "🍔 Buy Poochcam icons to support the devs 🍔"),
        String(localized: "🙈 Buy Poochcam icons to hide this message 🙈"),
        String(localized: "🙏 Buy Poochcam icons please =) 🙏"),
    ].randomElement()!
}
...
final class Model: NSObject, ObservableObject, @unchecked Sendable {
    ...
    @Published var poochcamCaptureError: String?
    ...
    let poochcamConfiguration = PoochcamConfigurationStore()
    var poochcamPublishingURL: String?

    override init() {
        super.init()
        if !PoochcamProfile.enabled {
            settings.load()
            streamingHistory.load()
            replaysStorage.load()
        }
        preparePoochcamProfile()
        setCurrentStream()
        updateIsPortrait()
    ...
    private func makeBuyIconsToastIfNeeded() -> Bool {
        if PoochcamProfile.enabled { return false }
    ...
    func setup() {
        ...
        if !PoochcamProfile.enabled { setupAppIntents() }
        ...
        if !PoochcamProfile.enabled { setupSampleBufferReceiver() }
        reloadStream()
        resetSelectedScene()
        DispatchQueue.main.async {
            self.setupAudio()
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               object: nil)
        if !PoochcamProfile.enabled { GCController.startWirelessControllerDiscovery {} }
        reloadLocation()
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               object: nil)
        if !PoochcamProfile.enabled, WCSession.isSupported() {
        ...
        if !PoochcamProfile.enabled {
            ...
            PoochcamShortcuts.updateAppShortcutParameters()
        }
        ...
        setQuickButton(type: .poochcamInMouth, isOn: database.face.showPoochcam)
        ...
        updateTalkback()
        if #available(iOS 18.0, *), !PoochcamProfile.enabled {
    ...
    func reloadNtpClient() {
        guard !PoochcamProfile.enabled else { return }
        stopNtpClient()
    ...
    private func setupSampleBufferReceiver() {
        ...
        sampleBufferReceiver.start(appGroup: poochcamAppGroup)
    }
    ...
    private func handle10sTimer() {
        ...
        if !PoochcamProfile.enabled { keepSpeakerAlive(now: monotonicNow) }
    }
    ...
    func findScoreboardPlayer(id: UUID) -> String {
        database.scoreboardPlayers.first(where: { $0.id == id })?.name ?? "🇸🇪 Poochcam"
    }
    ...
    private func updateBatteryLevel() {
        ...
        if battery.level <= 0.07, !isBatteryCharging(), !isMac(), battery.level != -1 {
            ...
            if database.chat.botEnabled, database.chat.botSendLowBatteryWarning {
                sendChatMessage(message: "Poochcam bot: \(lowBatteryMessage)")
            }
        }
    }
    ...
    func setPoochcamInMouth(on: Bool) {
        database.face.showPoochcam = on
        setFilterQuickButton(type: .poochcamInMouth, on: on)
        updateFaceFilterSettings()
    }

    func togglePoochcamInMouth() {
        database.face.showPoochcam.toggle()
        toggleFilterQuickButton(type: .poochcamInMouth)
        updateFaceFilterSettings()
    }
