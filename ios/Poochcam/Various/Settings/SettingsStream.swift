...
enum SettingsStreamSrtImplementation: String, Codable, CaseIterable {
    // Persist the legacy value; toString() supplies the current display name.
    case poochcam = "Moblin"
    case official = "Official"

    func toString() -> String {
        switch self {
        case .poochcam:
            String(localized: "Poochcam")
        case .official:
...
class SettingsStreamSrt: Codable, ObservableObject {
    ...
    @Published var implementation: SettingsStreamSrtImplementation = .poochcam
    ...
    required init(from decoder: any Decoder) throws {
        ...
        implementation = container.decode(.implementation, SettingsStreamSrtImplementation.self, .poochcam)
