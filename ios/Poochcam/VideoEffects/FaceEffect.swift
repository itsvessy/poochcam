...
final class FaceEffect: VideoEffect, @unchecked Sendable {
    ...
    private let poochcamImage: EffectImageCgImage?
    ...
    override init() {
        if let image = UIImage(named: "AppIconNoBackground"), let image = image.cgImage {
            poochcamImage = image.toEffectImage()
        } else {
            poochcamImage = nil
        }
    }
    ...
    private func makeMouthLayers(_ image: MTIImage,
                                 _ detections: [VNFaceObservation]) -> [MTILayer]
    {
        guard let poochcamImage = poochcamImage?.getMetalPetalImage() else {
            return []
        }
        let size = image.extent.size
        return detections.compactMap { detection in
            guard let mouth = calcMouth(detection, size, poochcamImage.extent.size) else {
                return nil
            }
            return .init(content: poochcamImage,
    ...
    private func calcMouth(_ detection: VNFaceObservation,
                           _ imageSize: CGSize,
                           _ poochcamImageSize: CGSize) -> CGRect?
    {
        ...
        let height = poochcamImageSize.height * (diffX / poochcamImageSize.width)
    ...
    private func addMouth(image: CIImage?, detections: [VNFaceObservation]?) -> CIImage? {
        guard let image, let detections, let poochcamImage = poochcamImage?.getCiImage() else {
            return image
        }
        var outputImage = image
        for detection in detections {
            guard let mouth = calcMouth(detection, image.extent.size, poochcamImage.extent.size) else {
                continue
            }
            let scale = mouth.width / poochcamImage.extent.width
            outputImage = poochcamImage
