...
final class AlertsEffect: VideoEffect, @unchecked Sendable {
    ...
    private func executePositionFace(
        ...
        for detection in faceDetections {
            ...
            let poochcamImage = alertImage.scaled(x: scale, y: scale)
            let centerPoint = rotatePoint(
                point: .init(x: placement.center.x - poochcamImage.extent.midX,
                             y: placement.center.y - poochcamImage.extent.midY),
