...
private func poochcamScript() -> String {
    loadStringResource(name: "poochcam", ext: "js")
}
...
private enum MessageToPoochcam: Codable {
    case ping
    case publish(message: PublishMessage)
    case subscribe(topic: SubscribeTopic)

    static func fromJson(data: String) throws -> MessageToPoochcam {
        guard let data = data.data(using: .utf8) else {
            throw "Not a UTF-8 string"
        }
        return try JSONDecoder().decode(MessageToPoochcam.self, from: data)
    }
}
...
class BrowserEffectServer: NSObject {
    ...
    private let poochcamAccess: Bool
    weak var delegate: (any BrowserEffectServerDelegate)?

    init(configuration: WKWebViewConfiguration, poochcamAccess: Bool) {
        self.poochcamAccess = poochcamAccess
        super.init()
        configuration.userContentController.addUserScript(.init(
            source: poochcamScript(),
            injectionTime: .atDocumentStart,
            forMainFrameOnly: false
        ))
        configuration.userContentController.add(self, name: "poochcam")
        // Existing third-party widgets may still post to this handler.
    ...
    private func send(message: MessageToBrowser) {
        ...
        poochcam.handleMessage("\(data)")
        """)
    }

    private func handleMessage(message: String) throws {
        do {
            switch try MessageToPoochcam.fromJson(data: message) {
            case .ping:
                handlePing()
